...
describe("E2E tests", function () {
  ...
  describe("Catalog [ 20 Points ]", () => {
    it("show empty catalog [ 2.5 Points ]", async () => {
      ...
      const titles = await page.$$eval("#catalog-page article h3", (t) =>
    ...
    it("show catalog [ 7.5 Points ]", async () => {
      ...
      const titles = await page.$$eval("#catalog-page h3", (t) =>
        t.map((s) => s.textContent)
      );
    ...
    it("show details [ 5 Points ]", async () => {
      ...
      expect(await page.isVisible(`strong:has-text("${data.category}")`)).to.be
        .true;
    ...
    it("guest does NOT see edit/delete buttons [ 5 Points ]", async () => {
      ...
      await page.waitForSelector("#catalog-page");
      await page.click(
        `.article-preview:has-text("${mockData.catalog[0].title}")`
      );
      await page.waitForTimeout(interval);
  ...
  describe("CRUD [ 40 Points ]", () => {
    ...
    it("non-author does NOT see delete and edit buttons [ 2.5 Points ]", async () => {
      ...
      await page.waitForSelector("#catalog-page");
      await page.click(
        `.article-preview:has-text("${mockData.catalog[1].title}")`
      );
      await page.waitForTimeout(interval);
  ...
  describe("Home Page [ 15 Points ]", async () => {
    it("all categories filled [ 5 Points ]", async () => {
      ...
      expect(
        await page.textContent('section.recent:has-text("JavaScript")')
      ).to.contains(arts[0].title);
      expect(
        await page.textContent('section.recent:has-text("C#")')
      ).to.contains(arts[1].title);
      expect(
        await page.textContent(
          'section.recent:has-text("Java"):not(:has-text("JavaScript"))'
        )
      ).to.contains(arts[2].title);
      expect(
        await page.textContent('section.recent:has-text("Python")')
      ).to.contains(arts[3].title);
  ...
  describe("Search Page [ 5 Points ]", async () => {
    ...
    it("show results [ 2.5 Points ]", async () => {
      await createHandler(endpoints.search("arrays"), {
      ...
      const matches = await page.$$eval(".article-preview h3", (t) =>
        t.map((s) => s.textContent)
      );
...
async function setupContext(context) {
  ...
  await createHandler(
    endpoints.home,
    { get: mockData.catalog.slice(0, 4) },
    context
  );
...
async function createHandler(match, handlers = {}, context) {
  ...
  function urlPredicate(current) {
    const url = (current instanceof URL
      ? current.href
      : current.url()
    ).toLowerCase();
